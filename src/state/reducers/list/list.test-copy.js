...
describe('list reducer', () => {
  describe('should return the initial state', () => {
    test('initialState', () => {
      // $FlowExpectedError
      const action: Action = {}
      const next: AllState = {siths: {byId: {}, ids: []}, planet: null}
      // $FlowExpectedError
  ...
  describe('should handle SITH_RECEIVED', () => {
    test('SITH_RECEIVED, initialState', () => {
      ...
      const siths = {byId: {[sith.id]: {...sith, lastUpdated} }, ids: [sith.id]}
      const planet = null
      const action: Action = actions.sithReceived(serverSith)
      const next: AllState = {planet, siths}
      const actual: State = reducer(initialState, action, next)
      const ids     = [3616, 1489, null, null, null]
      const added   = [1489]
      const removed = []
      const expected = {ids, added, removed}
